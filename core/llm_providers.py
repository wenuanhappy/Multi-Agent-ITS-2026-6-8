...
适配说明（Multi-Agent-ITS / CrewAI 版）：
  · Claude + DeepSeek 多 Provider 链式调用
  · 本模块无跨包 import，无需改 import 路径
  · CrewAI 自身走 litellm 调 LLM；本模块供 flow 层中需要绕过 CrewAI
    直接调 LLM 的场景使用（如分类器、诊断、anti-spoiler 等轻量调用）
"""
...
class MiniMaxProvider(LLMProvider):
    ...
    def complete(self, system, user, *, max_tokens=800, temperature=0.4, json_mode=False,
                 system_blocks: Optional[List[dict]] = None):
        # MiniMax 不支持 cache_control —— 把 blocks 合并成字符串
        if system_blocks is not None and len(system_blocks) > 0:
            system = "\n\n".join(
        ...
        for p in self.providers:
            ...
            for attempt in range(1, 10):  # 实际上限由 break 控制
                try:
                    ...
                    print(f"[LLM] {p.name} ok (attempt {attempt})")
                    last_provider.set(p.name)
                    return text
                except AuthError as e:
                    # **配置错误**——立刻 raise，不要静默吞，不要切下一个
                    print(f"[LLM] {p.name} AUTH error: {e}")
                    raise
                except BillingError as e:
                    print(f"[LLM] {p.name} billing error, switching: {e}")
                    last_exc = e
                    break
                    ...
                    if attempt < max_attempts:
                        ...
                        print(f"[LLM] {p.name} rate limit ({attempt}/{max_attempts}), retry in {wait}s")
                        time.sleep(wait)
                        continue
                    print(f"[LLM] {p.name} rate limit exhausted, switching")
                    break
                    ...
                    if attempt < max_attempts:
                        wait = _backoff(attempt)
                        print(f"[LLM] {p.name} transient error ({attempt}/{max_attempts}): {e}; retry in {wait}s")
                        time.sleep(wait)
                        continue
                    print(f"[LLM] {p.name} repeated failure, switching: {e}")
                    break
                except (BadRequestError, ValidationError) as e:
                    print(f"[LLM] {p.name} content/validation error, switching: {e}")
                    last_exc = e
                    break
                except Exception as e:
                    # 未分类异常 —— 保守当成可切（防止整个流程挂死）
                    print(f"[LLM] {p.name} unclassified: {type(e).__name__}: {e}")
                    last_exc = e
                    break
        # 所有 provider 跑完都没成功
...
def build_default_multi_provider(verbose: bool = True) -> Optional[MultiProvider]:
    ...
    if MINIMAX_API_KEY:
        try:
            ...
            if verbose: print(f"[LLM] loaded MiniMaxProvider (model={MINIMAX_MODEL})")
        except Exception as e:
            if verbose: print(f"[LLM] MiniMax init failed: {e}")
    ...
    if dk:
        ...
        try:
            ...
            if verbose: print(f"[LLM] loaded DeepSeekProvider flash (model={flash_model}, thinking=disabled)")
        except Exception as e:
            if verbose: print(f"[LLM] DeepSeek init failed: {e}")

    if not providers:
        if verbose: print("[LLM] no providers available (MINIMAX_API_KEY and DEEPSEEK_API_KEY both missing)")
        return None
    if verbose:
        print(f"[LLM] chain length {len(providers)}: {' -> '.join(p.name for p in providers)}")
